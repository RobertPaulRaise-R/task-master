...
interface CalendarDaysProps {
  ...
  todayActual: Date; // Actual current date for highlighting "today"
}
...
function CalendarDays({
  ...
  const weekdayOfFirstDay = firstDayOfMonth.getDay(); // 0 for Sunday, 1 for Monday, etc.
  ...
  for (let i = 0; i < 42; i++) {
    ...
    dayIterator.setDate(dayIterator.getDate() + 1); // Move to the next day
  }
    ...
    <div className="grid h-full grid-cols-7 grid-rows-6 border-r border-gray-200 dark:border-gray-600">
      {daysInGrid.map((day) => {
        ...
        if (day.selected) {
          ...
          // Using a pseudo-element for today's border to avoid layout shifts with ring
          cellClasses +=
        ...
        return (
          <div
            key={day.date.toISOString()} // Unique key for each day cell
